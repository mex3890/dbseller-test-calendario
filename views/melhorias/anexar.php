...
<section class="form">
    ...
    <form id="create_area_form">
        <input hidden value="<?= $_GET['area']; ?>" type="text" name="area" readonly class="field">
        <div class="mb-3">
            <label for="tarefa" class="form-label">Tarefa</label>
            <textarea required class="form-control field" name="tarefa" id="tarefa" aria-describedby="tarefa_help" style="resize: none"></textarea>
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control field" id="descricao" aria-describedby="descricao_help"
                      style="resize: none" required name="descricao"></textarea>
            <div id="descricao_help" class="form-text">Entre com a descrição da nova tarefa.</div>
        </div>
        <div class="mb-3">
            <div class="form-check">
                <input class="form-check-input field" type="checkbox" id="is_legal" name="is_legal">
        ...
        <div class="mb-3">
            ...
            <input max="<?= date('Y') . '-12-31'; ?>" min="<?= date('Y-m-d'); ?>" type="date" class="field form-control"
                   id="prazo_acordado" aria-describedby="prazo_acordado_help" name="prazo_acordado" required>
        ...
        <div class="mb-3">
            ...
            <input name="prazo_legal" disabled max="<?= date('Y') . '-12-31'; ?>" min="<?= date('Y-m-d'); ?>" type="date"
        ...
        <div class="mb-3">
            ...
            <select class="form-select field" aria-label="tendencia_help" id="tendencia" name="tendencia">
                ...
                <?php foreach (Tendencia::getInstance()->getAll() as $tendencia) { ?>
                    <option value="<?= $tendencia->id; ?>"><?= $tendencia->descricao; ?></option>
                <?php } ?>
            </select>
            <div id="gravidade_help" class="form-text">Entre com a tendência da tarefa.</div>
        </div>
        <button id="melhoria_create" type="button" class="btn btn-primary mt-3 px-5">Salvar</button>
    </form>
</section>